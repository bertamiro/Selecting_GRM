---
title: "Simulation of detection of L-shaped genes"
author: "BM"
date: "8/28/2019"
...
inst.all("mmotaF","miriammota",force.install = F)
inst.all("anaStatsUEB","uebvhir",force.install = F)
inst.all("simstudy")
inst.all("Lheuristic", "bertamiro")
```
...
ggplot(data = dta1x, aes(x=methylation, y=expression401)) +
  geom_point(color = "grey65", size = 0.75) +
  geom_smooth(se=FALSE, color="blue", size = 1, method = "auto") +ylim(0,10)+
  theme(panel.grid.minor = element_blank())
...
ggplot(data = dta4x, aes(x=methylation, y=expression1400)) +
  geom_point(color = "grey65", size = 0.75) +
  geom_smooth(se=FALSE, color="blue", size = 1, method = "auto") +ylim(0,10)+xlim(0,1)+
  theme(panel.grid.minor = element_blank())
...
plot(met10[2,], expr10[2,])
plot(met10[920,], expr10[920,])
